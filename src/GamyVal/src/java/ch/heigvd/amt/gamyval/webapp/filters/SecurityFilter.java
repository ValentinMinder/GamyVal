package ch.heigvd.amt.gamyval.webapp.filters;
...
/**
 * Security filter is applied to every request to the web pages to .check if the
 * access is authorized or it requires login. If it is protected then redirects
...
public class SecurityFilter implements Filter {
     ...
     * if the user has not been authenticated.
        ...
        if (path.startsWith("/static/")) {
            ...
            isTargetUrlProtected = false;
        } else if (path.startsWith("/pages/account") || path.startsWith("/account")) {
            // to allow account creation 
            isTargetUrlProtected = false;
        } else if (path.startsWith("/pages/home") || path.startsWith("/home")) {
            // to disable home page
